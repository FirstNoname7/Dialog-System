...
namespace MaryDialogSystem.Windows
{
    public class MeetingRoom : EditorWindow //отвечает за визуальную сторону редактора (визуалка моей переговорной)
    {
        ...
        private void AddToolbar() //добавить верхнюю панель переговорной
        {
            ...
            loadButton = ElementUtility.CreateButton("Загрузить", ()=>Load()); //создаю кнопку для загрузки того, что сохраняли в переговорной
            ...
            toolbar.Add(loadButton); //добавляю к тулбару кнопку загрузки того, что сохраняли в переговорной
        ...
        private void Load()
        {
            ...
            {
                return; //прост возвращаемся, нам нечего загружать
            }
            ...
            InputOutputUtilities.Initialize(graphView, Path.GetFileNameWithoutExtension(filePath)); //инициализируем граф, который надо загрузить (graphView) с именем filePath (без расширения, т.е. без .asset)
